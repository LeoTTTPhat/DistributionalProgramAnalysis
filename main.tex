...
\subsection{Artifact smoke test}
To make the prototype reproducible before the full benchmark run, we
implemented the Phase~0 smoke test from the project plan as a deterministic
local artifact. The command \texttt{make smoketest} analyzes 20 fixed
\texttt{rolling\_max} samples, writes per-sample invariants and posterior
survival rates to \texttt{runs/smoketest/}, and evaluates the
\texttt{DPA-filter} application at $\tau=0.85$. Table~\ref{tab:smoketest}
reports the current run. The smoke test is not a substitute for the
HumanEval+ pilot; its purpose is to validate the end-to-end plumbing and
to ensure that the paper's motivating example has executable support.

\begin{table}[t]
\centering
\caption{Deterministic artifact smoke test on the \texttt{rolling\_max}
motivating example. Results are generated by \texttt{make smoketest}.}
\label{tab:smoketest}
\begin{tabular}{@{}lc@{}}
\toprule
Metric & Value \\
...
Samples analyzed & 20 \\
Smoke tests & 7 \\
Unique inferred invariants & 8 \\
Mean invariants per sample & 7.7 \\
Raw sample pass rate & 85.0\% \\
DPA survivors at $\tau=0.85$ & 17 \\
Survivor pass rate & 100.0\% \\
Wall-clock time & 0.018s \\
\bottomrule
\end{tabular}
\end{table}
